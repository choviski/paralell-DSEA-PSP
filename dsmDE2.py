...
class dsm2:
    ...
    def new_indiv_frag(self, ind, n=100, temp=2.0):
        ...
        seq.add_mover(self.rosetta_conf.get_mer('9'))
    ...
    def evolutionaryCycle(self, target, seed, psc):
        F_new, CR_new = self.de_service.autoAjuste(target.getF(), target.getCR())
    ...
    def otimiza(self):
        ...
        test = teste(self.D, self.rosetta_conf.ss3)
        time_start = time.time()

        pool = mp.Pool(processes=7, initializer=test.call_initializer, initargs=(self.rosetta_conf,))
        while self.avaliacoes < self.maxAval:    
            ...
            for i in range(0, len(self.especies)):         # linhas = especies
                ...
                #resultados = Parallel(n_jobs=4)(delayed(self.evolutionaryCycle)(seed,self.especies[i],psc,j) for j in range(0, len(self.especies)))
                
                # colunas = individuos da especie
                #for a in self.especies[i]:   
                ind , avaliacoes = test.otimiza(seed,self.especies[i],psc,self.rosetta_conf,pool)
                self.avaliacoes += avaliacoes
                self.pop = self.pop + ind

                    #self.pop.append(self.evolutionaryCycle(a,seed,psc))
                
                for _ in self.especies[i]:
